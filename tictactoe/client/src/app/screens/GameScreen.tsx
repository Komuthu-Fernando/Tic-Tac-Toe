...
export default function GameScreen() {
  ...
  useEffect(() => {
    ...
    socket.on("room_state", (data: any) => {
      ...
      const playerMarks = data.players as Record<number, Mark>;
      setYourMark(playerMarks[userId] ?? null);

      const loadedPlayers: Player[] = Object.keys(playerMarks).map((idStr) => {
        const id = parseInt(idStr);
        return {
          userId: id,
          username: id === userId ? username : `Player ${id}`,
          mark: playerMarks[id],
          avatar: `https://i.pravatar.cc/50?u=${id}`,
        };
      });
    ...
    socket.on("error_msg", (err: any) => console.warn("Server error:", err.message));
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-r from-black via-purple-900 to-blue-900 p-4">
      ...
      <div className="flex gap-6 mb-6">
        {players.map((p) => (
          ...
          >
            <img src={p.avatar} alt="avatar" className="w-12 h-12 rounded-full mb-2" />
